...
const { MENU_ACTIONS, SERVICES } = require('../config/constants');
...
const mainMenuKeyboard = (user = {}) => {
    ...
    const buttons = [
        [
            { text: 'Order OTP',   callback_data: MENU_ACTIONS.ORDER_OTP, style: 'primary' },
            { text: 'Isi Saldo',   callback_data: MENU_ACTIONS.TOPUP,     style: 'success' },
        ],
        [
            { text: 'Referral',    callback_data: 'referral',   style: 'success' },
            { text: 'Top User',    callback_data: 'top_users',  style: 'primary' },
        ],
        [
            { text: 'Riwayat Order',   callback_data: 'history_orders',       style: 'primary' },
            { text: 'Riwayat Deposit', callback_data: 'history_transactions',  style: 'primary' },
        ],
        [
            { text: 'Profil',        callback_data: MENU_ACTIONS.PROFILE },
    ...
    if (user.role === 'admin') {
        buttons.push([
            { text: 'Panel Admin', callback_data: 'admin_panel', style: 'primary' },
        ]);
    }
...
const profileMenuKeyboard = (user = {}) => {
    const buttons = [
        ...
        [{ text: 'Refresh',       callback_data: MENU_ACTIONS.REFRESH, style: 'success' }],
...
const serviceSelectionKeyboard = () => {
    const buttons = [
        [{ text: SERVICES.WHATSAPP.name, callback_data: `select_service:${SERVICES.WHATSAPP.code}`, style: 'primary' }],
        [{ text: SERVICES.TELEGRAM.name, callback_data: `select_service:${SERVICES.TELEGRAM.code}`, style: 'primary' }],
        [{ text: '« Kembali',            callback_data: MENU_ACTIONS.MAIN_MENU }],
    ];
    return { reply_markup: { inline_keyboard: buttons } };
};
...
const topupConfirmationKeyboard = (amount) => ({
    reply_markup: {
        inline_keyboard: [[
            { text: 'Konfirmasi',  callback_data: `topup_confirm:${amount}`,   style: 'success' },
...
const paymentLinkKeyboard = (url) => ({
    reply_markup: {
        inline_keyboard: [
            ...
            [{ text: '« Menu Utama',   callback_data: MENU_ACTIONS.MAIN_MENU  }],
        ]
    }
});
